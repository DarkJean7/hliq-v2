...
console.log(NL + "-- All Accounts sums the wallets own figures --")
{
  ...
  const inAll = await waitFor(q, 'All Accounts', () => /All Accounts/i.test(document.body.textContent ?? ''), null, 90000)
  ok('the combined view is up', inAll)
  await q.evaluate(() => window.mobVTab('portfolio'))
  const got = await waitFor(q, 'the combined Net PnL', () => /-\$3,643\.12/.test(document.getElementById('mobVContent')?.textContent ?? ''), null, 90000)
