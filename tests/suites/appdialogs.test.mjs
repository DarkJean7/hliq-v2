...
console.log(nl + '-- the fill sound --')
{
  ...
  t('off really is silent', SOUNDS.off.notes.length === 0 && !isOn(mem()))
  t('there are sounds to choose from', Object.keys(SOUNDS).length >= 5)
  t('each is a label and some notes', Object.values(SOUNDS).every(s => s.label && Array.isArray(s.notes)))

  const st = mem()
  t('a choice is remembered', setSound('chime', st) === 'chime' && soundName(st) === 'chime' && isOn(st))
...
console.log(nl + '-- wired to the fills, and to Settings --')
{
  ...
  t('the desktop row is there', /id="fillSoundSel"/.test(html) && /id="fillSoundVol"/.test(html))
