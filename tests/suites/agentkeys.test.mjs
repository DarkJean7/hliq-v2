...
console.log(nl + '-- one account decides what the key UI shows --')
{
  ...
  t('the sync reads that account’s key only', sync.includes("localStorage.getItem(_agentKeyForAddr(addr))"))
...
console.log(nl + '-- and every reader asks that account, not state.addr --')
{
  ...
  t('and the desktop Settings row', cli.includes("const agentKey = _agentKeyInView() || localStorage.getItem('hliq_agent_key')"))
}
...
console.log(nl + '-- saving writes to the account in view --')
{
  ...
  t('auto-generate stores under the account it generated for',
    cli.includes('localStorage.setItem(_agentKeyForAddr(_acct), privateKey)'))
